...
(function($) {
	...
	var Mapplic = function() {
		...
		self.init = function(el, params) {
			...
			self.el = el.addClass('mapplic-element mapplic-loading');
			...
			$.ajax({
				url: '/controller/coffee/areaJson',
				type: 'GET',
...
/*			$.getJSON(self.o.source, function(data) { // Success
		...
		function Tooltip() {
			...
			this.init = function() {
				...
				// Construct
				this.el = $('<div class="admin-form tab-pane mw200 active mapplic-tooltip" id="register1" role="tabpanel"></div>').addClass('theme-danger');
				$('<a></a>').addClass('mapplic-tooltip-close').attr('href', '#').click(function(e) {
					e.preventDefault();
					self.deeplinking.clear();
					s.hide();
				}).appendTo(this.el);
				// this.mainDiv = $('<div class="admin-form tab-pane mw200 active" id="register1" role="tabpanel"></div>').addClass('theme-danger').appendTo(this.el);
				this.header = $('<div class="panel panel-danger heading-border"><div class="panel-heading"></div></div>').appendTo(this.el).children();
				this.form = $('<form method="post" action="#" id="form-login1"><div class="panel-body row center-block"><div class="row"><div class="center-block" id="action_list"><button class="btn btn-system btn-alt btn-block" data-effect="mfp-flipInY">Flip In Y</button><button class="btn btn-primary btn-alt btn-block" data-effect="mfp-zoomIn">Zoom In</button><button class="btn btn-alert btn-alt btn-block" data-effect="mfp-rotateUp">Rotate Up</button><button class="btn btn-info btn-alt btn-block" data-effect="mfp-slideUp">Slide Up</button><button class="btn btn-warning btn-alt btn-block" data-effect="mfp-sign">Sign Flip</button><button class="btn btn-danger btn-alt btn-block" data-effect="mfp-fullscale">Fullscale</button></div></div></div><div class="panel-footer"><button type="submit" class="button btn-primary">Sign in</button></div></form>').appendTo(this.el);
				this.title = $('<span class="panel-title-dialog"><i class="fa fa-pencil-square"></i></span>').appendTo(this.header);
		...
		function Sidebar() {
			...
			this.init = function() {
				...
				this.list = $('<ul></ul>').addClass('mapplic-list').attr('role','tablist').appendTo(listContainer);
				this.notfound = $('<p></p>').addClass('mapplic-not-found').text('Nothing found. Please try a different search.').appendTo(listContainer);
				if (!self.o.search) listContainer.css('padding-top', '0');
				$('<div></div>').addClass('mapplic-contact mapplic-sidebar-add mt50 well p10').appendTo(listContainer);
			}
			...
			this.addCategories = function(categories) {
				...
				$.each(categories, function(index, category) {
					...
					var ul = $('<ul></ul>').css('border-color', category.color).appendTo(item);
					if (category.show == 'false') ul.hide();
			...
			this.addLocation = function(data) {
				...
				var link = $('<a></a>').attr('href', '#' + data.id).attr('role', 'tab').attr('data-toggle','tab').appendTo(item);
				if (data.thumbnail) $('<img>').attr('src', data.thumbnail).addClass('mapplic-list-thumbnail').appendTo(link);
				$('<h4></h4>').text(data.title).appendTo(link)
				$('<span></span>').html(data.about).appendTo(link);
				var category = $('.mapplic-list-category.' + data.category);

				if (category.length) $('ul', category).append(item);
		...
		function TableDetail() {
			this.init = function(data) {
				var wrap = $('<div></div>').addClass('tab-content mw900 center-block center-children').appendTo(self.container);
				$.each(data.levels, function(index, value) {
					$.each(value.locations, function(index, location) {
						var statusClass;
						switch (location.statusCode) {
							case "0":
								statusClass = "success";
								break;
							case "1":
						...
						var el = $('<div></div>').addClass('admin-form theme-'+statusClass+' tab-pane').attr('id', location.id).attr('role','tabpanel').appendTo(wrap);
						var container = $('<div></div>').addClass('panel panel-'+statusClass+' heading-border').appendTo(el);
						var title = $('</div><div class="panel-heading"><span class="panel-title"><i class="fa fa-sign-in"></i>Table '+ location.id +'</span></div></div>').appendTo(container);
						var form = $('<form method="post" action="/">' +
							'<div class="panel-body p25 pt10">' +
							'<div class="section row">' +
							'<div class="admin-form col-md">' +
							'<div class="section-divider mv40">'+location.status+'</div>' +
							'<div class="section">' +
							'<a href="#" class="admin-form button btn-social facebook span-left btn-block">' +
							'<span>' +
							'<i class="fa fa-facebook"></i>' +
							'</span>' +
							'Facebook' +
							'</a>' +
							'<a href="#" class="admin-form button btn-social twitter span-left btn-block">' +
							'<span>' +
							'<i class="fa fa-twitter"></i>' +
							'</span>' +
							'Twitter' +
							'</a>' +
							'<a href="#" class="admin-form button btn-social googleplus span-left btn-block">' +
							'<span>' +
 ...
 */
			...
			self.tableinfo = new TableDetail();
			self.tableinfo.init(data);
